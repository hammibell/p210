...
def number_of_vehicle():
    ...
    bot_vehicles = world.get_actors().filter('audi car.*')
    threading.Timer(0.1, number_of_vehicle).start()
    threading.Timer(0.4, count).start()
    ...
    if len(bot_vehicles) > 1:
        ...
        for x in : bot_vehicles #complete the loop by adding bot car variable
            if  != x.id:#compare with id
                get_distance_of_bot_vehicles.((distance(x.get_location()), x)).append()#append list
